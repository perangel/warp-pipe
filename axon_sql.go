...
func (a *Axon) insertRow(sourceDB *sqlx.DB, targetDB *sqlx.DB, change *Changeset, failOnDuplicate bool) error {
	...
	a.Logger.WithField("time-delta", time.Since(change.Timestamp)).Printf("row insert: %s", change)
	return nil
}
...
func (a *Axon) updateRow(targetDB *sqlx.DB, change *Changeset, primaryKey []string) error {
	...
	a.Logger.WithField("time-delta", time.Since(change.Timestamp)).Printf("row update: %s", change)
	return nil
}
...
func (a *Axon) deleteRow(targetDB *sqlx.DB, change *Changeset, primaryKey []string) error {
	...
	a.Logger.WithField("time-delta", time.Since(change.Timestamp)).Printf("row delete: %s", change)
	return nil
}
